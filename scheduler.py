...
KW_MAX_BID = {
    ...
    # 에어컨수리 1000원→2490회, 가스충전 1000원→3485회 → 상한 대폭 상향
    "에어컨수리":          5000,
    "에어컨가스충전":      5000,
    "에어컨냉매충전":      3000,
    "에어컨가스":          3000,
...
INIT_BIDS = {
    # 2026-06-16 estimate API 기반 재설정 — 노출 시작가 기준
    # (스윗스팟 탐색 시 하한선 역할: 이 이하로는 내려가지 않음)
    # 에어컨수리: 700원→1415회 노출 확인 → 시작점 700원
    "에어컨수리":          {"ms": 12000, "comp": "높음", "bid": 700},
    # 에어컨가스충전: 700원→1064회 → 시작점 700원
    "에어컨가스충전":      {"ms": 8000,  "comp": "높음", "bid": 700},
    # 에어컨냉매충전: 700원→649회 → 시작점 700원
    "에어컨냉매충전":      {"ms": 5000,  "comp": "높음", "bid": 700},
    # 에어컨가스: 500원→461회 → 시작점 500원
    "에어컨가스":          {"ms": 4000,  "comp": "높음", "bid": 500},
    # 삼성에어컨수리: 500원→70회 → 시작점 500원
    "삼성에어컨수리":      {"ms": 3000,  "comp": "높음", "bid": 500},
    # LG에어컨수리: 1000원→112회 → 시작점 1000원
    "LG에어컨수리":        {"ms": 2500,  "comp": "높음", "bid": 1000},
    # 에어컨점검: 700원→90회 → 시작점 700원
    "에어컨점검":          {"ms": 2000,  "comp": "중간", "bid": 700},
    # 에어컨매립배관: 300원→95회 → 시작점 300원
    "에어컨매립배관":      {"ms": 1500,  "comp": "중간", "bid": 300},
    # 에어컨매립배관수리: 500원→71회 → 시작점 500원
    "에어컨매립배관수리":  {"ms": 1200,  "comp": "중간", "bid": 500},
    # 에어컨매립배관교체: 500원→97회 → 시작점 500원
    "에어컨매립배관교체":  {"ms": 900,   "comp": "중간", "bid": 500},
    # 에어컨물떨어짐: 500원→151회 → 시작점 500원
    "에어컨물떨어짐":      {"ms": 700,   "comp": "중간", "bid": 500},
}
