...
KW_MAX_BID = {
    ...
    "에어컨매립배관교체":  1000,
    "에어컨물떨어짐":      2000,
    # 2026-06-16 신규 틈새 키워드 (estimate API 기반) — 변곡점 분석 후 상한 상향
    "에어컨냉매":          5000,   # 2000→5000: 변곡점 700원, 스윗스팟 탐색 여지
    "에어컨실외기소음":    5000,   # 2000→5000: 변곡점 700원, 스윗스팟 탐색 여지
    "에어컨전기세":        5000,   # 2000→5000: 변곡점 1,000원, 스윗스팟 탐색 여지
    "에어컨냉매가스":      3000,   # 1000→3000: 변곡점 1,500원, 스윗스팟 탐색 여지
    "에어컨얼음":          1000,   # 유지: 소량 검색, 현 500원 유지
    "에어컨성에":           700,   # 유지: 소량 검색
    "대우에어컨수리":       700,   # 유지: 소량 검색
    "위니아에어컨수리":     700,   # 유지: 소량 검색
    # 2차 틈새 키워드 — 변곡점 분석 후 상한 상향
    "에어컨실외기안돌아감": 2000,  # 700→2000: 변곡점 500원
    "에어컨소음":          3000,   # 700→3000: 변곡점 1,000원
    "실외기소음":          3000,   # 700→3000: 변곡점 700원
    "에어컨안켜짐":        2000,   # 700→2000: 변곡점 700원
    "에어컨드레인":        2000,   # 700→2000: 변곡점 700원
    "에어컨고장":          5000,   # 1000→5000: 변곡점 1,500원
    "에어컨가스충전비용":  5000,   # 1500→5000: 변곡점 1,000원
    "에어컨바람안나옴":    1000,   # 유지: 변곡점 이미 도달
    "에어컨물":            3000,   # 1000→3000: 변곡점 1,500원
    "창문형에어컨수리":     500,   # 유지: 소량 검색
    "에어컨시원하지않음":   500,   # 유지: 소량 검색
    "에어컨전원안켜짐":     500,   # 유지: 소량 검색
    "실외기고장":          3000,   # 1000→3000: 변곡점 2,000원
}
...
INIT_BIDS = {
    ...
    "에어컨냉매":          {"ms": 5000,  "comp": "낮음", "bid": 700},   # 500→700: 변곡점 700원(1,701회↑)
    "에어컨실외기소음":    {"ms": 3000,  "comp": "낮음", "bid": 700},   # 500→700: 변곡점 700원(1,865회↑)
    "에어컨전기세":        {"ms": 2000,  "comp": "낮음", "bid": 1000},  # 700→1000: 변곡점 1,000원(1,153회↑)
    "에어컨냉매가스":      {"ms": 1000,  "comp": "낮음", "bid": 1500},  # 500→1500: 변곡점 1,500원(187회↑)
    "에어컨얼음":          {"ms": 800,   "comp": "낮음", "bid": 500},   # 유지: 변곡점 이미 도달
    "에어컨성에":          {"ms": 600,   "comp": "낮음", "bid": 300},   # 300원→80회
    "대우에어컨수리":      {"ms": 500,   "comp": "낮음", "bid": 300},   # 300원→188회
    "위니아에어컨수리":    {"ms": 400,   "comp": "낮음", "bid": 300},   # 300원→99회
    # 2026-06-16 2차 틈새 키워드 (증상/고장 계열) — 변곡점 분석 후 bid 상향 적용
    "에어컨실외기안돌아감": {"ms": 2000, "comp": "낮음", "bid": 500},  # 300→500: 변곡점 500원(722회↑)
    "에어컨소음":          {"ms": 1500, "comp": "낮음", "bid": 1000}, # 300→1000: 변곡점 1,000원(345회↑)
    "실외기소음":          {"ms": 1500, "comp": "낮음", "bid": 700},  # 300→700: 변곡점 700원(470회↑)
    "에어컨안켜짐":        {"ms": 2000, "comp": "낮음", "bid": 700},  # 300→700: 변곡점 700원(429회↑)
    "에어컨드레인":        {"ms": 1000, "comp": "낮음", "bid": 700},  # 300→700: 변곡점 700원(233회↑)
    "에어컨고장":          {"ms": 3000, "comp": "낮음", "bid": 1500}, # 500→1500: 변곡점 1,500원(216회↑)
    "에어컨가스충전비용":   {"ms": 1000, "comp": "낮음", "bid": 1000}, # 700→1000: 변곡점 1,000원(239회↑)
    "에어컨바람안나옴":     {"ms": 800,  "comp": "낮음", "bid": 500},  # 유지: 변곡점 이미 도달
    "에어컨물":            {"ms": 800,  "comp": "낮음", "bid": 1500}, # 500→1500: 변곡점 1,500원(95회↑)
    "창문형에어컨수리":     {"ms": 500,  "comp": "낮음", "bid": 300},  # 유지: 소량 검색
    "에어컨시원하지않음":   {"ms": 500,  "comp": "낮음", "bid": 300},  # 유지: 소량 검색
    "에어컨전원안켜짐":     {"ms": 500,  "comp": "낮음", "bid": 300},  # 유지: 소량 검색
    "실외기고장":          {"ms": 1000, "comp": "낮음", "bid": 2000}, # 500→2000: 변곡점 2,000원(105회↑)
}
...
def do_check():
    ...
    # 단, 전날 DB에 기록 자체가 없는 신규 키워드는 상향 허용 (has_yest와 무관)
    if not has_yest and not budget_critical:
        # 전날 DB에 데이터가 전혀 없는 경우(완전 신규) → 상향 허용
        # 전날 DB는 있는데 imp=0인 경우(노출 0 확인됨) → 스킵
        if len(yest_db) > 0:
            log("  ℹ️  전날 실노출 없음(DB 있음) → 자동 조정 스킵 (현재 입찰가 유지, 데이터 축적 대기)")
            return True
        else:
            log("  ℹ️  전날 DB 없음(신규/리셋) → 노출 없는 키워드만 상향 허용")
